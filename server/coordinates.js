...
function getCoordinates(address){
  ...
  return axios.get('https://maps.googleapis.com/maps/api/geocode/json', {
    ...
    .then(function (response) {
      // console.log('this is response in coordinates', response.data.results[0]);
...
function getPlaces(coordinates,radius,type, name){
  ...
  console.log('coordinates', coordinates, 'radius', radius, 'type', type);
  return axios.get('https://maps.googleapis.com/maps/api/place/nearbysearch/json', {
    params: {
      ...
      key: process.env.IP.APIKEY
    }
  })
  .then(function (response) {
    console.log('this is places', response);
...
function getPlacesFromThirdPoint(address1, address2, category, maxTime) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    .then(function(resp) {
      ...
      console.log('coordinates thirdPoint', response.thirdPoint.coordinates);
